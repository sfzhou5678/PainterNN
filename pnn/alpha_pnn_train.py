from pnn.model.AlphaRPNNModel import AlphaRPNNModel
...
def num_train():
  ...
  with tf.name_scope('Train'):
    with tf.variable_scope("Model", reuse=None, initializer=initializer):
      train_model = AlphaRPNNModel(config=config, is_training=True, GO_ID=GO_ID)

  with tf.name_scope('Valid'):
    with tf.variable_scope("Model", reuse=True):
      val_model = AlphaRPNNModel(config=config, is_training=False, GO_ID=GO_ID)
