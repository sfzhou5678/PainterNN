...
class AlphaRPNNModel:
  ...
  def pnn_decode(self, encoder_outputs, encoder_final_state, hidden_units, is_training, rnn_layers=2,
    ...
    with tf.variable_scope("decoder", ):
      ...
      decoder_cell, decoder_init_state = build_decoder_cell(None, use_attention=False)
      ...
      second_decoder_cell, second_decoder_init_state = build_decoder_cell(pixel_latent_list, use_attention=False)
      state = second_decoder_init_state
      last_pixel_vec = tf.zeros(
        [self.config.batch_size, self.config.width, self.config.height, self.config.word_embedding_size],
        dtype=tf.float32)
      with tf.variable_scope("SecondRNN"):
        # 为句子中的每一个单词建模,共执行maxNumSteps次
        for time_step in range(self.config.max_num_steps):
          if time_step > 0:
            tf.get_variable_scope().reuse_variables()
